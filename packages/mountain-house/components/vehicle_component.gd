...
enum VehicleMode {Wander, Seek, Arrive, Still, None}
...
func _vehicle_physics_process(delta):
	match curr_mode:
		VehicleMode.None:
			pass
		VehicleMode.Still:
	...
	# still overrides all
	if curr_mode == VehicleMode.Still:
		actor.velocity = Vector2.ZERO
	if actor.velocity.length() > 0.5 and do_rotate:
