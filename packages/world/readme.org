...
: T
** 2024-07-27
*** notes on a "tile server"
How do x/y/z coordinates work in a typical tile server?

In a typical tile server, x/y/z coordinates work as follows:

1. z: Zoom level (0 is zoomed out, higher numbers zoom in)
2. x: Horizontal tile position from left to right (0 to 2^z - 1)
3. y: Vertical tile position from top to bottom (0 to 2^z - 1)

These coordinates form a quadtree structure, allowing efficient retrieval and display of map tiles at various zoom levels and locations.

so x and y are integers pointing to tiles, not coordinates?

Correct. In a tile server system:

1. x and y are indeed integers pointing to specific tiles, not geographic coordinates.
2. Each tile is a square image, typically 256x256 pixels.
3. The x/y integers identify which tile to fetch at a given zoom level (z).
4. To convert between geographic coordinates (latitude/longitude) and tile coordinates (x/y/z), mathematical formulas are used.

This system allows for efficient storage, retrieval, and display of map data at various scales.

Why 256 by 256 pixels?

The 256x256 pixel size for map tiles is common for several reasons:

1. Power of 2: 256 is 2^8, which aligns well with computer memory and processing.
2. Balance: It's a good compromise between file size and detail.
3. Performance: Small enough for quick loading, large enough to reduce the number of HTTP requests.
4. Zoom levels: Fits nicely with the quadtree structure used in tile systems.
5. Screen resolutions: Works well across various device screens.
6. Historical precedent: Widely adopted by early map providers like Google Maps.

While 256x256 is standard, some systems use other sizes (e.g., 512x512) for specific needs.

What is z again? does it make sense to say something like 1 1/2 1/4 1/8 1/16 1/32...1/n?

z represents the zoom level in tile systems:

1. z is always an integer, typically ranging from 0 to about 20-22.
2. It doesn't use fractional values like 1/2, 1/4, 1/8, etc.

The zoom levels work as follows:

- z = 0: Entire world in one tile
- z = 1: World divided into 4 tiles (2x2)
- z = 2: World divided into 16 tiles (4x4)
- ...and so on, with each increment doubling the number of tiles in each dimension

The progression you mentioned (1/2, 1/4, 1/8...) actually relates to the scale of each tile relative to the whole map, rather than the zoom level itself. As z increases, each tile represents a smaller fraction of the world's surface.
*** applying it to us
The map size has been decided at 25,600, which means that a scale of 1/100 with dimensions 256x256 will get us a tile at 0,0 in zoom level 0.

At 1/50 a map of dimensions 512 is created. Or potentially 4 tiles with 256 offsets zoom level 1.  X and y will always translate to some multiple of 256 offset.

What happens if we ask for a coordinate outside available tiles? Simply wrap around 2^z where z is zoom level.

0 1/100, 1 1/50
** 2024-07-28
So basically today, we first create a world tile function which takes Z/X/Y coordinates all the way up to zoom level 8. That is now just a quick function standalone that lives in the world package. Then we created a new package called world explorer, which right now is just injecting, plugging in, that previous function into leaflet jhas. But we have plans you can see below to roll our own with pixie. I think we have an idea around that and then we can go through there. Basically we want to roll our own because we have to inside it that it's going to be all seamless and that pixie will immediately turn into produced tile maps. We also want to do something like I want to be able to change something like a world configuration on the server and have that update pushed out to the client at least for debugging purposes.
*** convo on ts leaflet
**** I need to make a leaflet.js/google maps like thing with typescript and pixi.js.  I already have a tileserver that servers png files based on z/x/y coordinates.

Great! You're already halfway there with your tileserver set up. To create a Google Maps-like interface using TypeScript and PixiJS, you'll need to follow these general steps:

1. Set up your project with TypeScript and PixiJS
2. Create a basic PixiJS application
3. Implement a camera system for panning and zooming
4. Load and display tiles from your tileserver
5. Handle user interactions (drag to pan, scroll to zoom)
...
class MapViewer {
    ...
    private setupInteraction(): void {
        ...
        this.app.stage.on('pointermove', (event: PIXI.InteractionEvent) => {
            if (isDragging) {
                ...
                this.tileContainer.x += dx;
                this.tileContainer.y += dy;
        ...
        this.app.view.addEventListener('wheel', (event: WheelEvent) => {
            ...
            this.zoom = Math.max(0, Math.min(18, this.zoom + zoomChange));
            this.loadTiles();
        });
    }
    ...
    private loadTile(x: number, y: number, z: number): void {
        const tileKey = `${z}/${x}/${y}`;
        if (!this.tileContainer.getChildByName(tileKey)) {
            const texture = PIXI.Texture.from(`http://your-tileserver.com/${tileKey}.png`);
    ...
    private getViewportBounds(): { minX: number, maxX: number, minY: number, maxY: number } {
        ...
        const viewportWidth = this.app.screen.width / (this.tileSize * scale);
        const viewportHeight = this.app.

**** I think we should probably keep a separate tile layer for each zoom level, but they only need to be a layer the size such that if the total screen is 8 tiles across, the tile layer should be 10 tiles.

You're absolutely right. That's a great optimization strategy. By maintaining separate layers for each zoom level and limiting the size of each layer, we can significantly improve performance and memory usage. Let's modify our approach to implement this idea:
...
class MapViewer {
    private app: PIXI.Application;
    private tileLayers: Map<number, PIXI.Container>;
    private zoom: number = 0;
    private tileSize: number = 256;
    private viewportTiles: number = 8;
    private layerTiles: number = 10;
    ...
    private setupInteraction(): void {
        // ... (keep the interaction setup from the previous example)
    }

    private loadTiles(): void {
        if (!this.tileLayers.has(this.zoom)) {
            const layer = new PIXI.Container();
            this.tileLayers.set(this.zoom, layer);
            this.app.stage.addChild(layer);
        }

        const layer = this.tileLayers.get(this.zoom)!;
        const scale = Math.pow(2, this.zoom);
        const tileOffset = Math.floor(this.layerTiles / 2) - Math.floor(this.viewportTiles / 2);

        const centerX = -layer.x / (this.tileSize * scale);
        const centerY = -layer.y / (this.tileSize * scale);

        const startX = Math.floor(centerX) - tileOffset;
        const startY = Math.floor(centerY) - tileOffset;

        for (let x = 0; x < this.layerTiles; x++) {
            for (let y = 0; y < this.layerTiles; y++) {
                const tileX = startX + x;
                const tileY = startY + y;
                this.loadTile(tileX, tileY, this.zoom, layer);
            }
        }

        // Remove tiles that are now out of view
        layer.children.forEach((tile: PIXI.Sprite) => {
            const tileX = Math.floor(tile.x / this.tileSize);
            const tileY = Math.floor(tile.y / this.tileSize);
            if (tileX < startX || tileX >= startX + this.layerTiles ||
                tileY < startY || tileY >= startY + this.layerTiles) {
                layer.removeChild(tile);
            }
        });

        // Hide other zoom levels
        this.tileLayers.forEach((layerContainer, zoomLevel) => {
            layerContainer.visible = (zoomLevel === this.zoom);
        });
    }

    private loadTile(x: number, y: number, z: number, layer: PIXI.Container): void {
        const tileKey = =${z}/${x}/${y}=;
        if (!layer.getChildByName(tileKey)) {
            const texture = PIXI.Texture.from(=http://your-tileserver.com/${tileKey}.png=);
            const tile = new PIXI.Sprite(texture);
            tile.name = tileKey;
            tile.x = x * this.tileSize;
    ...
    public update(): void {
        // Call this method in your game loop or when the map needs to be updated
        this.loadTiles();
    }
}
#+end_src
...
1. Maintains separate tile layers for each zoom level.
2. Limits each layer to a 10x10 grid of tiles (configurable via =layerTiles=).
3. Centers the visible tiles around the current viewport.
4. Removes tiles that are
