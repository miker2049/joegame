...
case $1 in
    linux)
        do_build
        DEPLOY_GTK_VERSION=3 linuxdeploy --appimage-extract-and-run --executable=bin/worldtool \
                    --custom-apprun=package/AppRun \
                    --icon-file=package/icon.png \
                    --desktop-file=package/worldtool.desktop \
                    --plugin gtk \
                    --appdir=appimage $(find bin -name "lib*" -printf "-l%p ")
        cp bin/worldtool appimage/usr/bin
        cp -R Resources appimage/usr
        appimagetool --appimage-extract-and-run --comp xz -g appimage "worldtool-${VERSION}.AppImage"
        ;;
    ...
    macos)
        ...
        xxd -ps -c1 bin/worldtool | tr -d '\n' | sed "s/$replace_fr/$replace_to/" | fold -w 2 | xxd -r -p > "$contents/MacOS/worldtool"
        chmod +x "$contents/MacOS/worldtool"

        hdiutil create -quiet -srcfolder "$bundle" out.dmg
        # NOTE: ULMO = lzma compression = Catalina+ only
        hdiutil convert -quiet out.dmg -format ULMO -o "worldtool-${VERSION}.dmg"
        rm out.dmg
        ;;
