...
static void recompute_freq(MidSong *song, int v) {
  int sign =
          (song->voice[v].sample_increment < 0), /* for bidirectional loops */
  ...
  a = TIM_FSCALE(
      ((double)(song->voice[v].sample->sample_rate) *
       (double)(song->voice[v].frequency)) /
          ((double)(song->voice[v].sample->root_freq) * (double)(song->rate)),
      FRACTION_BITS);
...
static void recompute_amp(MidSong *song, int v) {
  ...
  tempamp =
      (song->voice[v].velocity * song->channel[song->voice[v].channel].volume *
  ...
  if (!(song->encoding & PE_MONO)) {
    if (song->voice[v].panning > 60 && song->voice[v].panning < 68) {
      ...
      song->voice[v].left_amp = TIM_FSCALENEG(
          (double)(tempamp)*song->voice[v].sample->volume * song->master_volume,
          21);
      ...
      song->voice[v].left_amp = /* left_amp will be used */
          TIM_FSCALENEG((double)(tempamp)*song->voice[v].sample->volume *
                            song->master_volume,
                        20);
    } else {
      ...
      song->voice[v].left_amp = TIM_FSCALENEG(
          (double)(tempamp)*song->voice[v].sample->volume * song->master_volume,
          27);
      song->voice[v].right_amp =
          song->voice[v].left_amp * (song->voice[v].panning);
...
extern void start_note(MidSong *song, MidEvent *e, int i) {
  ...
  if (ISDRUMCHANNEL(song, e->channel)) {
    if (!(ip = song->drumset[song->channel[e->channel].bank]
                   ->instrument[e->a])) {
  ...
  song->voice[i].tremolo_phase_increment =
      song->voice[i].sample->tremolo_phase_increment;
  ...
  song->voice[i].vibrato_control_ratio =
      song->voice[i].sample->vibrato_control_ratio;
  ...
  while (i--) {
    if (song->voice[i].status == VOICE_FREE)
      lowest = i; /* Can't get a lower volume than silence */
             ...
             (song->voice[i].note == e->a ||
              song->channel[song->voice[i].channel].mono))
      kill_note(song, i);
  }
...
static void note_off(MidSong *song) {
  ...
  while (i--)
    if (song->voice[i].status == VOICE_ON &&
        song->voice[i].channel == e->channel && song->voice[i].note == e->a) {
...
static void all_sounds_off(MidSong *song) {
  ...
  while (i--)
    if (song->voice[i].channel == c && song->voice[i].status != VOICE_FREE &&
        song->voice[i].status != VOICE_DIE) {
      kill_note(song, i);
    }
}
...
size_t mid_song_read_wave(MidSong *song, sint8 *ptr, size_t size) {
  ...
  while (song->current_sample < end_sample) {
    ...
    if (song->current_event->time > end_sample)
      ...
      compute_data(song, &ptr,
                   song->current_event->time - song->current_sample);
  }
