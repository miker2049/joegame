...
export default function (level: ILevelComponents): void {
  ...
  level.scene.physics.world.addCollider(
    level.npcs,
    level.npcs,
    (npc1, npc2) => {
    ...
    level.scene.physics.world.addCollider(
      ...
      (npc1, npc2) => {
        level.machineRegistry.sendTo('player_machine', {
          type: 'BUMP',
          dir: touchToDir(npc2.body.touching)
        })
        level.machineRegistry.sendTo(npc2.name + '_machine', {
          type: 'BUMP',
          sprite: npc1
  ...
  level.scene.physics.world.setBounds(
    0,
    0,
    level.map.widthInPixels,
    level.map.heightInPixels
  )
  if (level.player?.charBody) level.player.charBody.setCollideWorldBounds(true)
}
