...
/**
 * Modified helper function to hash an integer using the above permutation table
 * and a seed value
 *
 * @param[in] seed Seed value to use in hashing
 * @param[in] i    Integer value to hash
 *
 * @return 8-bits hashed value
 */
static inline uint8_t hash(uint32_t seed, int32_t i) {
  uint8_t t = perm[(i ^ seed) & 0xFF];
  return perm[(seed + t) & 0xFF];
 ...
 * coordinates.
 */
float SimplexNoise::noise(float x, size_t seed) {
  ...
  t0 *= t0;
  n0 = t0 * t0 * grad(hash(seed, i0), x0);

  // Calculate the contribution from the second corner
  float t1 = 1.0f - x1 * x1;
  //  if(t1 < 0.0f) t1 = 0.0f; // not possible
  t1 *= t1;
  n1 = t1 * t1 * grad(hash(seed, i1), x1);
 ...
 * coordinates.
 */
float SimplexNoise::noise(float x, float y, size_t seed) {
  ...
  const int gi0 = hash(seed, i + hash(seed, j));
  const int gi1 = hash(seed, i + i1 + hash(seed, j + j1));
  const int gi2 = hash(seed, i + 1 + hash(seed, j + 1));
 ...
 * coordinates.
 */
float SimplexNoise::noise(float x, float y, float z, size_t seed) {
  ...
  int gi0 = hash(seed, i + hash(seed, j + hash(seed, k)));
  int gi1 = hash(seed, i + i1 + hash(seed, j + j1 + hash(seed, k + k1)));
  int gi2 = hash(seed, i + i2 + hash(seed, j + j2 + hash(seed, k + k2)));
  int gi3 = hash(seed, i + 1 + hash(seed, j + 1 + hash(seed, k + 1)));
 ...
 * coordinates.
 */
float SimplexNoise::fractal(size_t octaves, size_t seed, float x) const {
  float output = 0.f;
  float denom = 0.f;
 ...
 * coordinates.
 */
float SimplexNoise::fractal(size_t octaves, size_t seed, float x,
                            float y) const {
  float output = 0.f;
  float denom = 0.f;
  float frequency = mFrequency;
 ...
 * coordinates.
 */
float SimplexNoise::fractal(size_t octaves, size_t seed, float x, float y,
                            float z) const {
  float output = 0.f;
  float denom = 0.f;
  float frequency = mFrequency;
  ...
  for (size_t i = 0; i < octaves; i++) {
    output +=
        (amplitude * noise(x * frequency, y * frequency, z * frequency, seed));
    denom += amplitude;
