"use strict";

var _regeneratorRuntime2 = require("@babel/runtime/regenerator");

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = _callee;

var _regenerator = _interopRequireDefault(require("@babel/runtime/regenerator"));

var _Platform = _interopRequireDefault(require("../components/Platform"));

var _marked = /*#__PURE__*/_regeneratorRuntime2.mark(_callee);

function _createForOfIteratorHelper(o, allowArrayLike) { var it; if (typeof Symbol === "undefined" || o[Symbol.iterator] == null) { if (Array.isArray(o) || (it = _unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = o[Symbol.iterator](); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it.return != null) it.return(); } finally { if (didErr) throw err; } } }; }

function _unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return _arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return _arrayLikeToArray(o, minLen); }

function _arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }

function _callee(level, layer) {
  var platformSets, _iterator, _step, obj_, plat, _platform$, _platform$2, platform, platDur, platConfig;

  return _regenerator.default.wrap(function _callee$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          if (level.map.getObjectLayer(layer)) {
            _context.next = 2;
            break;
          }

          return _context.abrupt("return");

        case 2:
          platformSets = {};
          _iterator = _createForOfIteratorHelper(level.map.getObjectLayer(layer).objects);

          try {
            for (_iterator.s(); !(_step = _iterator.n()).done;) {
              obj_ = _step.value;

              if (platformSets[obj_.name]) {
                platformSets[obj_.name].push(obj_);
              } else {
                platformSets[obj_.name] = [obj_];
              }
            } // now we iterate through our new object and create the npcs

          } catch (err) {
            _iterator.e(err);
          } finally {
            _iterator.f();
          }

          _context.t0 = _regenerator.default.keys(platformSets);

        case 6:
          if ((_context.t1 = _context.t0()).done) {
            _context.next = 16;
            break;
          }

          plat = _context.t1.value;
          platform = platformSets[plat];
          platDur = 1000;

          if (platform[0].properties) {
            platform[0].properties.forEach(function (prop) {
              if (prop.name === 'speed') {
                platDur = prop.value;
              }
            });
          }

          platConfig = {
            level: level,
            x: platform[0].x / level.map.tileWidth,
            y: platform[0].y / level.map.tileHeight,
            width: platform[0].width / level.map.tileWidth,
            height: platform[0].height / level.map.tileHeight,
            endX: ((_platform$ = platform[1]) === null || _platform$ === void 0 ? void 0 : _platform$.x) / level.map.tileWidth || platform[0].x / level.map.tileWidth,
            endY: ((_platform$2 = platform[1]) === null || _platform$2 === void 0 ? void 0 : _platform$2.y) / level.map.tileHeight || platform[0].y / level.map.tileHeight,
            name: platform[0].name,
            speed: platDur || 1,
            ptype: platform[0].type || "default"
          };
          _context.next = 14;
          return new _Platform.default(platConfig);

        case 14:
          _context.next = 6;
          break;

        case 16:
        case "end":
          return _context.stop();
      }
    }
  }, _marked);
}