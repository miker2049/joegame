    ...
    let action = tiled.registerAction("align-object", () => {
        ...
        if (tiled.activeAsset.selectedObjects.length > 0) {
            tiled.activeAsset.selectedObjects.forEach((object) => {
                object.x = closestMultiple(
                    object.x,
                    tiled.activeAsset.tileWidth
                );
                object.y = closestMultiple(
                    object.y,
                    tiled.activeAsset.tileHeight
                );
                object.width = closestMultiple(
                    object.width,
                    tiled.activeAsset.tileWidth
                );
                object.height = closestMultiple(
                    object.height,
                    tiled.activeAsset.tileHeight
                );
            });
