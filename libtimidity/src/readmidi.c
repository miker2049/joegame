...
static sint32 getvl(MidIStream *stream) {
  ...
  uint8 c;
  for (;;) {
    if (!mid_istream_read(stream, &c, 1, 1))
      return l;
    l += (c & 0x7f);
    if (!(c & 0x80))
      return l;
...
static int read_meta_data(MidIStream *stream, MidSong *song, sint32 len,
                          uint8 type) {
...
static MidEvent *groom_list(MidSong *song, sint32 divisions, sint32 *eventsp,
  ...
  groomed_list = lp =
      (MidEvent *)timi_calloc(sizeof(MidEvent) * (song->event_count + 1));
  if (!groomed_list) {
    song->oom = 1;
    free_midi_list(song);
    return NULL;
...
MidEvent *read_midi_file(MidIStream *stream, MidSong *song, sint32 *count,
                         sint32 *sp) {
